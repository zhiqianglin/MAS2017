    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        database.getReference().child("users").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for(String userID:objectMap.keySet()){
        ...
        System.out.println("print outside: " + " " + new Date());
        mref.child("users").child(user.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            @Override
            public void onDataChange(DataSnapshot snapshot) {
                if (snapshot.getValue() == null) {
                    //user exists, do something
                    System.out.println("time: " + snapshot.getValue() + " " + new Date());
                    mref.child("users").child(user.getUid()).child("emailAdress").setValue(user.getEmail());
                    mref.child("users").child(user.getUid()).child("createdTime").setValue(df.format(new Date()));
                    mref.child("users").child(user.getUid()).child("preferredName").setValue(user.getEmail().split("@")[0]);
                    mref.child("users").child(user.getUid()).child("figureImage").setValue("false");
                }
            }
        ...
        mref.child("users").child(user.getUid()).child("lastSignIn").setValue(df.format(new Date()));
